...
String? _emailAddress(List<CodeableConcept>? medium) {
  final emailIndex = medium?.indexWhere(
      (element) => element.coding?.first.code?.toString() == 'EMAILWRIT');
...
String? _phoneNumber(List<CodeableConcept>? medium) {
  final phoneIndex = medium?.indexWhere(
      (element) => element.coding?.first.code?.toString() == 'SMSWRIT');
